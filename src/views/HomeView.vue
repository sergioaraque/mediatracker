<template>
  <div class="min-h-screen text-white flex relative">
    ...
    <main class="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 pb-28 md:pb-8">
      ...
      <Transition name="fade-down">
        <div
          v-if="mediaError && !media.loading && media.all.length > 0"
          class="mb-6 rounded-2xl border border-rose-500/25 bg-rose-500/10 p-4 flex items-start gap-3"
        >
          <div class="rounded-lg bg-rose-500/20 p-2 shrink-0">
            <WifiOff class="w-4 h-4 text-rose-300" />
          </div>
          <div class="flex-1 min-w-0">
            <p class="text-sm font-semibold text-rose-100">No se pudo actualizar la colección</p>
            <p class="text-xs text-rose-200/80 mt-0.5">{{ mediaError }}</p>
          </div>
          <button
            @click="loadMedia"
            class="shrink-0 rounded-xl border border-rose-500/25 bg-rose-500/10 px-3 py-2 text-xs font-bold text-rose-200 hover:bg-rose-500/20 transition-colors"
          >
            Reintentar
          </button>
        </div>
      ...
      <HomeInsights
        ...
        :recommended-now="recommendedNow"
        :next-in-queue="nextInQueue"
        :recent-media="recentMedia"
        @open-tool="openTool"
        @open-detail="openDetail"
        @remove-from-queue="q.remove"
      />
      ...
      <div v-else-if="media.all.length === 0 && mediaError" class="flex flex-col items-center justify-center py-32 text-center">
        <div class="w-20 h-20 rounded-3xl bg-gradient-to-br from-rose-500/20 to-red-500/20 border border-rose-500/20 flex items-center justify-center mb-6">
          <WifiOff class="w-9 h-9 text-rose-400" />
        </div>
        <h2 class="text-2xl font-bold text-white mb-2">No pudimos cargar tu colección</h2>
        <p class="text-gray-400 max-w-sm mb-2">{{ mediaError }}</p>
        <p class="text-gray-500 text-sm max-w-sm mb-8">Pulsa reintentar para volver a consultar Appwrite.</p>
        <div class="flex flex-wrap justify-center gap-2">
          <button @click="loadMedia" class="btn-primary px-6 py-3 flex items-center gap-2">
            <RefreshCw class="w-5 h-5" /> Reintentar
          </button>
          <button @click="openTool('stats')" class="btn-secondary px-5 py-3 flex items-center gap-2">
            <BarChart2 class="w-4 h-4" /> Ver estadísticas
          </button>
        </div>
      </div>
...
import { Inbox, Plus, SearchX, Trash2, X, Compass, Sparkles, ListOrdered, CalendarDays, BarChart2, Dices, Upload, Lightbulb, Keyboard, RefreshCw, WifiOff } from 'lucide-vue-next'
